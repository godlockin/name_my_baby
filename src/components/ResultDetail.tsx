  ...
  return (
    ...
    >
      ...
      >
        {/* Header */}
        <div
          className="sticky top-0 z-10 flex items-center justify-between p-6 border-b bg-[var(--color-bg)] border-[rgba(196,69,54,0.1)]"
        >
          <h2
            id="drawer-title"
        ...
        <div className="p-6 space-y-6">
          {/* Name Display */}
          <div className="text-center p-6 rounded-lg bg-[rgba(196,69,54,0.05)]">
            <div className="text-5xl font-bold mb-2 text-[var(--color-primary)]">
              {name.name}
            </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="text-center p-4 rounded-lg border border-[rgba(196,69,54,0.2)]">
              <div className="text-2xl font-bold text-[var(--color-primary)]">
                {name.score}
              </div>
              <div className="text-xs text-gray-500 mt-1">综合评分</div>
            </div>
            <div className="text-center p-4 rounded-lg border border-[rgba(212,175,55,0.3)]">
              <div className="text-2xl font-bold text-[var(--color-accent)]">
                {name.culturalScore}
              </div>
              <div className="text-xs text-gray-500 mt-1">文化评分</div>
            </div>
            <div className="text-center p-4 rounded-lg border border-[rgba(44,44,44,0.2)]">
            ...
            <div className="flex flex-wrap gap-2">
              {name.wuxing.map((w, index) => (
                <span
                  ...
                  style={{ backgroundColor: getWuxingColor(w), color: "white" }}
                >
                  {w}
                </span>
              ))}
          ...
          <div>
            ...
            <div className="grid grid-cols-4 gap-2">
              {name.bazi.split(" ").map((char, index) => (
                <div
                  key={index}
                  className="text-center p-3 rounded-lg bg-[rgba(212,175,55,0.1)]"
                >
